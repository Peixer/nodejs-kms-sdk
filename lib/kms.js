...
class KmsClient extends KmsBase {
   ...
   *   - Digest {string} required: original message
   *   - KeyId {string} required: global unique identifier
   *   - KeyVersionId {string} required: version ID of key
   *   - AliasName {string} required: the alias to be operated, prefix must be 'alias/'
   */
  async AsymmetricSign(query, runtime) {
    const $runtime = {
      timeout: 10000,
      timeouted: 'retry',
      retry: {
   ...
   *   - keyVersionId {string} required: version ID of key
   */
  async GetPublicKey(query, runtime) {
    const $runtime = {
      timeout: 10000,
      timeouted: 'retry',
      retry: {
